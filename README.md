...
Паттерны и подходы:
- Разделение по слоям (Presentation / Domain / Infrastructure).
- Композиция вместо глубокой иерархии наследования.
- Слабое связывание между слоями: orchestration в index.ts, типизированные события (types/events.ts).
- Инкапсуляция взаимодействия с внешним API в ApiClient (components/apiClient.ts).
